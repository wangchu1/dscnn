...
% global variables helps RealtimeDS
global stack;
stack =java.util.Stack();
global forward_only_flag;
...
% 2nd pass for finding permute layer in CW customized pooling.
%if opts.customizedPooling == true,
%    viewpoolIdx = find(cellfun(@(x)strcmp(x.name, 'permute1'),net.layers));
%    if ~isempty(viewpoolIdx),
%        if numel(viewpoolIdx)>1,
%            error('More than one viewpool layers found!');
%        end
...
im0 = randn(net.meta.normalization.imageSize(1), ...
...
%	pattern = 'ClusterPool';
%	if (~isempty(strfind(layers.name{fi},pattern)))
%	    layers.name{fi}
%	    continue;
%	else
%           featCell{fi}(:,:,:,ri:nSubWins:end) = single(gather(res(layers.index(fi)).x));
